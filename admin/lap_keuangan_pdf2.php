...
while ($datas=mysqli_fetch_array($sql2)) { 
	while ($data2=mysqli_fetch_array($sql3)) { 
		while ($data3=mysqli_fetch_array($sql4)) { 
	
			$total = $datas['total']-$data2['total'];              
			
			// $pdf->Cell(40,7,'Total Saldo Awal',1,0,'C');
			// $pdf->Cell(145,7,'Rp.'.number_format($data3['saldo_awal']).'',1,1,'C');
			$pdf->Cell(40,7,'Total Saldo Masuk',1,0,'C');
			$pdf->Cell(145,7,'Rp.'.number_format($datas['total']).'',1,1,'C');
			$pdf->Cell(40,7,'Total Saldo Keluar',1,0,'C');
			$pdf->Cell(145,7,'Rp.'.number_format($data2['total']).'',1,1,'C');
		}
	}          
}
$pdf->Cell(40,7,'Saldo Akhir',1,0,'C');
$pdf->Cell(145,7,'Rp.'.number_format($total).'',1,1,'C');
$pdf->Output();
